...
export default function Taxtable({ taxes: initialTaxes = [], isLoading = false }) {
  const columns = [
    ...
    { 
      ...
      render: (item) => {
        const taxTypeMap = {
          'Income Tax': { label: 'Income Tax', class: 'income-tax' },
          'VAT': { label: 'VAT', class: 'vat' },
          'Corporate Tax': { label: 'Corporate Tax', class: 'corporate' },
          'Other': { label: 'Other', class: 'other' }
        };
        const type = taxTypeMap[item.taxType] || { label: item.taxType, class: 'other' };
        return (
          <span className={`tax-type ${type.class} premium-badge`}>
            {type.label}
          </span>
        );
      }
    ...
    { 
      ...
      sortable: true,
      render: (item) => (
        <span className="amount premium-amount">${formatAmount(item.amount)}</span>
      )
    },
    { 
      key: 'status', 
      ...
      render: (item) => {
        ...
        return (
          <span className={`status-badge ${statusClass} premium-badge`}>
