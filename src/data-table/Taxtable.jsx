...
export default function Taxtable({ taxes: initialTaxes = [], isLoading = false }) {
  const columns = [
    ...
    { 
      ...
      render: (item) => {
        const taxTypeMap = {
          'Income Tax': { label: 'Gelir Vergisi', class: 'income-tax' },
          'VAT': { label: 'KDV', class: 'vat' },
          'Corporate Tax': { label: 'Kurumlar Vergisi', class: 'corporate' },
          'Other': { label: 'Diğer', class: 'other' }
        };
        const type = taxTypeMap[item.taxType] || { label: item.taxType, class: 'other' };
        return (
          <span className={`tax-type ${type.class}`}>
            {type.label}
          </span>
        );
      }
    ...
    { 
      ...
      render: (item) => {
        ...
        const statusClass = status === 1 ? 'status-paid' : 'status-unpaid';
        const statusMap = {
          1: 'Ödendi',
          2: 'Ödenmedi'
        };
        ...
        return (
          <span className={`status-badge ${statusClass}`}>
            {statusMap[status] || formatStatus(status)}
          </span>
        );
      }
    }
  ...
  const editFormFields = [
    ...
    { name: 'taxDetail', label: 'Vergi Detayı', type: 'text' },
    { 
      name: 'taxType', 
      label: 'Vergi Türü', 
      type: 'select',
      options: [
        { value: 'Income Tax', label: 'Gelir Vergisi' },
        { value: 'VAT', label: 'KDV' },
        { value: 'Corporate Tax', label: 'Kurumlar Vergisi' },
  ...
  return (
    <DataTable
      ...
      editFormTitle="Vergi Kaydı Düzenle"
      emptyMessage="Henüz Vergi Kaydı Yok"
      emptySubMessage="Lütfen vergi girişi eklemek için yeni bir kayıt oluşturun."
      luxuryMode={true}
    />
  );
}
