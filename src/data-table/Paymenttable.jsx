...
const DateIcon = () => (
  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ marginRight: '8px', filter: 'drop-shadow(0 0 2px rgba(212, 175, 55, 0.3))' }}>
    ...
    { key: 'paymentType', label: 'PAYMENT TYPE', icon: <TypeIcon />, sortable: true,
      render: (item) => {
        const paymentTypeMap = {
          ...
          'Nakit': { label: 'Cash', class: 'cash' },
          'Kredi Kartı': { label: 'Credit Card', class: 'credit' },
          'Banka Havalesi': { label: 'Bank Transfer', class: 'bank' }
        };
        const type = paymentTypeMap[item.paymentType] || { label: item.paymentType, class: 'other' };
        return <span className={`payment-type ${type.class} premium-badge`}>{type.label}</span>;
      }
    },
    { key: 'amount', label: 'AMOUNT', icon: <AmountIcon />, sortable: true,
      render: (item) => <span className="amount premium-amount">${formatAmount(item.amount)}</span> },
    { key: 'status', label: 'STATUS', icon: <StatusIcon />, sortable: false,
      render: (item) => {
        const statusMap = {
          1: { text: 'Received', class: 'status-receipt' },
          2: { text: 'Issued', class: 'status-given' }
        };
        ...
        return <span className={`status-badge ${status.class} premium-badge`}>{status.text}</span>;
      }
    }
  ];
